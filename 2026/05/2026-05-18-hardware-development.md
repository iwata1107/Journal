...
例えばESP32のセンサーロガーなら、最初の実物はこのくらいの規模でいい。

![ESP32センサーロガーの配線例](media/esp32-sensor-wiring.jpg "ESP32開発ボード、ブレッドボード、温湿度センサー、LED、USBケーブルをつないだ最小構成")

ここで見たいのは、センサーの種類そのものではなく、電源、GND、信号線、USB、ログの関係。

![ESP32センサーロガーの接続図](media/esp32-connection-map.svg "USBで書き込み、I2Cセンサーを読み、Wi-FiでUbuntuサーバーへ送る接続図")

この段階で詰まりやすいのは、GNDを共通にしていない、3.3Vの部品に5Vを入れる、SDA/SCLを逆にする、Wi-FiのSSIDや送信先URLが違う、というところ。

だから最初は、配線を増やさない。センサー1つ、LED1つ、サーバー送信1つに絞る。
...
サーバー側の実物イメージは、こういう小さいラボ構成で十分。

![ラボサーバーとデータフロー](media/lab-server-data-flow.jpg "センサーボード、ルーター、スイッチ、ミニサーバー、可視化用ノートPCをつないだラボ構成")

初心者がサーバー設定で詰まるのは、コードよりも「住所合わせ」が多い。

![Ubuntuサーバー設定マップ](media/server-settings-map.svg "ESP32、Docker Compose、Postgres、Grafanaで必要になるSSID、URL、ポート、DB設定の関係")

ESP32が見ているサーバーURL、Dockerが公開しているポート、Grafanaが読みに行くDB、MQTTのtopic名が少しでもズレると、全体として動かない。

なので、最初は設定値を散らさない。`.env` やメモに、SSID、サーバーIP、API URL、MQTT topic、DB URL、Grafana URLをまとめて書く。
...
Jetson系の開発キットは、こういう接続を一つずつ確認する。

![Jetson風エッジAI開発キットの接続例](media/jetson-camera-setup.jpg "エッジAIボード、カメラ、電源、Ethernet、HDMI、USB周辺機器をつないだ検証構成")

![Jetsonセットアップの確認マップ](media/jetson-setup-map.svg "JetPack、物理接続、Docker、推論ログの確認順序")

独自キャリアボードは、かなり後でいい。

最初から作るべきなのはJetson基板ではなく、Jetsonを使った検証環境。カメラ、照明、固定具、電源、ログ、温度、推論速度を測れる環境を先に作る。

ここで詰まりやすいのは、電源容量が足りない、カメラの向きが逆、JetPackとDockerイメージの組み合わせが合わない、冷却不足で遅くなる、というところ。

だからJetsonでは、モデルの精度を見る前に、温度、消費電力、FPS、メモリ、カメラ入力、ログ保存を先に見る。
...
![3Dプリント筐体のフィット確認](media/printed-enclosure-fit-check.jpg "3Dプリント筐体、基板、USB穴、ネジ柱、通気スリット、ノギスを使ったフィット確認")

ここで見るのは、見た目の完成度ではない。

基板のネジ穴が合うか。USBコネクタにケーブルが刺さるか。センサーが外気に触れるか。ボタンが押せるか。熱が逃げるか。ケースを閉めたときにケーブルを潰さないか。
...
Bring-upの机は、こういう状態になる。

![初回基板のBring-up測定セットアップ](media/measurement-bringup-setup.jpg "安定化電源、マルチメーター、ロジックアナライザ、ノートPCを基板につないだBring-up構成")

写真で見ると機材が多く見えるけど、役割は単純。

安定化電源は壊さないため。マルチメーターは電圧を見るため。ロジックアナライザは通信を見るため。PCはファームウェアを書いてログを見るため。
...
初回基板のBring-upは、この順番で進める。

![初回基板Bring-upの確認フロー](media/bringup-debug-flow.svg "目視、抵抗確認、電流制限、電圧測定、USB認識、UARTログ、I2C確認、通信確認の順序")

大事なのは、異常が出たら進まないこと。

電流が想定より大きい。電圧が出ない。ICが熱い。この3つが出たら、ファームウェアやサーバーの問題ではなく、まず電源と基板を見る。
...
ハードウェアで「何が分からないか分からない」ときは、だいたい次の6箇所で止まっている。

![初心者がつまずく接続と設定ポイント](media/common-stumble-map.svg "電源、信号線、USB、ネットワーク、サーバー設定、製造データの6つのつまずきポイント")
